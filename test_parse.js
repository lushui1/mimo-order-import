...
const fs = require('fs');
const buf = fs.readFileSync('D:/Download/GoogleDownload/demos/湖南仓.xlsx');
...
console.log('Row 2 (first data):', JSON.stringify(data[2]).slice(0,300));
const header = data[1];
...
for (const f of fields) {
  ...
  if (idx >= 0) {
    console.log('  Sample from row 2:', data[2][idx]);
  }
}
let count = 0;
...
console.log('Non-empty data rows:', count);
